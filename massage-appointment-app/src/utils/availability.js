...
// You can expand this to manage by therapist, or fetch from Firestore later.
const CLINIC_HOURS = {
  // Example: All days 9 AM to 5 PM
...
// Map service names to their durations in minutes
 ...
 * @returns {Array<moment.Moment>} - Array of moment objects representing start times of possible slots.
 */
export const generateAvailableSlots = (date, durationMinutes) => {
  ...
  while (currentSlot.clone().add(durationMinutes, 'minutes').isSameOrBefore(end)) {
    ...
    currentSlot.add(15, 'minutes'); // Increment by 15-minute intervals to find next possible start
                                    // You can adjust this granularity
  }
...
/**
 * Filters generated slots against already booked appointments.
 ...
 */
export const filterBookedSlots = (generatedSlots, bookedAppointments, durationMinutes, excludeAppointmentId = null) => {
  return generatedSlots.filter(slot => {
    const slotStart = slot.clone();
    const slotEnd = slot.clone().add(durationMinutes, 'minutes');

    // Check if this slot conflicts with any existing booked appointment
    const isConflict = bookedAppointments.some(bookedAppt => {
      ...
      const bookedEnd = bookedStart.clone().add(SERVICE_DURATIONS[bookedAppt.service] || 60, 'minutes'); // Use booked service duration

      // A conflict exists if the new slot overlaps with a booked slot
      // Overlap logic: (StartA < EndB) && (EndA > StartB)
      return slotStart.isBefore(bookedEnd) && slotEnd.isAfter(bookedStart);
    });

    return !isConflict && slotStart.isSameOrAfter(moment()); // Ensure slot is not a conflict and is in the future
  });
};